			...
			<div class="loginForm">
				<form id="loginform" name="loginform" method="post" class="niceform"
					...
					<div id="idInputLine" class="loginFormIpt showPlaceholder"
						...
						<input id="loginform:idInput" type="text" name="username" value="${username}" class="loginFormTdIpt" maxlength="50" />
						<label for="idInput" class="placeholder" id="idPlaceholder">帐号：</label>
					</div>
					<div class="forgetPwdLine"></div>
					<div id="pwdInputLine" class="loginFormIpt showPlaceholder">
						<input id="loginform:pwdInput" class="loginFormTdIpt" value="${password}" type="password" name="password" value="" />
						<label for="pwdInput" class="placeholder" id="pwdPlaceholder">密码：</label>
					</div>
					<div class="loginFormIpt loginFormIptWiotTh"
						style="margin-top:58px;">
						<span>
						<a style="margin-top: -30px" id="btn"  onclick="return checkName()" class="easyui-linkbutton" data-options="iconCls:'icon-search'">忘记密码</a>  
						<script>
							function checkName(){
								var username = $("input[name='username']").val();
								var flag = "1";
								$.ajax({
									url : '${pageContext.request.contextPath}/userAction_findUserByUsername.action',
									type : 'POST',
									dataType : 'json',
									async:false,
									...
									success : function(data) {
										if(data==1){
											location.href="${pageContext.request.contextPath}/message.jsp?username="+username;
										}else{
											flag="0";
										}
									},
								...
								if(flag==0){
									$.messager.alert("提示信息","账号不存在！请重新输入！");
						...
						<a style="margin-top:-34px;float:right;">
						<input type="submit" name="submit" value="登录" class="btn btn-login"/>
						</a>
						<!--忘记密码的功能  跳转到新的页面-->
