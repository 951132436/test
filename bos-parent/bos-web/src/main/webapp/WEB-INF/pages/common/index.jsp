...
<script type="text/javascript">
	...
	$(function() {
		var setting = {
			data : {
				simpleData : { // 简单数据 [{id:1,name:'a',pId:'0'}，{id:2,name:'a',pId:'1'}]
					enable : true
				}
			},
			callback : {
		...
		// 基本功能菜单加载
		$.ajax({
			url : '${pageContext.request.contextPath}/functionAction_findMenus.action',
			type : 'POST',
			dataType : 'text',
		...
		$("#btnEp").click(function(){
			//提交form表单、做表单校验
			var r = $("#editPwdForm").form('validate');
			if(r){
				//判断两次输入的密码是否一致
				...
				if(txtNewPass==txtRePass){
					//提交Form表单刷新页面、使用ajax
// 					$("#editPwdForm").submit();
					$.post("userAction_updatePwd.action",{"password":txtNewPass},function(data){
						if(data == '1'){
...
<body class="easyui-layout">
	<div data-options="region:'north',border:false"
		...
		<div id="dateStr" style="height: 20px;margin-top: 40px;font-size: 14px;" class="word_grey">  </div>
		<script>
		setInterval("document.getElementById('dateStr').innerHTML=new Date().toLocaleString()+' 星期'+'日一二三四五六'.charAt(new Date().getDay());",1000);
		</script>
    ...
    <div id="editPwdWindow" class="easyui-window" title="修改密码" collapsible="false" minimizable="false" modal="true" closed="true" resizable="false"
        ...
        <div class="easyui-layout" fit="true">
            <div region="center" border="false" style="padding: 10px; background: #fff; border: 1px solid #ccc;">
               <form id="editPwdForm" action="userAction_updatePwd.action" method="post">
                <table cellpadding=3>
                    <tr>
                        <td>新密码：</td>
                        <td><input id="txtNewPass" name="newPwd" type="Password" data-options="validType:'length[4,6]',required:true" class="txt01 easyui-validatebox" /></td>
                    </tr>
                    <tr>
                        <td>确认密码：</td>
                        <td>	
                        	<!-- url:校验地址规则
                        		 length:校验输入项长度
                        		 emil:校验邮箱规则 -->
                        	<input id="txtRePass" name="rePwd" type="password" data-options="validType:'length[4,6]',required:true" class="txt01 easyui-validatebox"/>
                        </td>
                    </tr>
                </table>
                </form>
