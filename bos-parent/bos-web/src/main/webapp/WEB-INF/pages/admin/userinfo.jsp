...
<script type="text/javascript">
	$(function(){
		...
		//发送请求获取角色数据
		$.post('roleAction_listajax.action',null,function(data){
			//从data 数组中获取角色数据，在页面上通过checkbox展示角色
			for(var i = 0;i<data.length;i++){
				var role = data[i];
				//alert(role.id);
				$("#roleTD").append('<input type="checkbox" name="rolesIds" id="'+role.id+'" value="'+role.id+'"></input><label for="'+role.id+'">'+role.name+'</label>');
			}
		});
	});
</script>	
...
<body class="easyui-layout" style="visibility:hidden;">
    ...
    <div region="center" style="overflow:auto;padding:5px;" border="false">
       <form id="form" action="userAction_save.action" method="post" >
           <table class="table-edit"  width="95%" align="center">
				...
				<tr>
	           	...
	           	<tr>
	           		<td>选择角色:</td>
	           		<td id="roleTD">
<!-- 	           			<input type="checkbox" name="roles" id="aaa"></input><label for="aaa">超级管理员</label> -->
           			</td>
           		</tr>
           </table>
       </form>
